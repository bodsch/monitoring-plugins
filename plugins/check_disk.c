...
main (int argc, char **argv)
{
  ...
  for (path = path_select_list; path; path=path->name_next) {
    ...
    if (fsp.fsu_blocks && strcmp ("none", me->me_mountdir)) {
      ...
      if (verbose >= 3) {
        sprintf ("For %s, used_pct=%g free_pct=%g used_units=%g free_units=%g total_units=%g used_inodes_pct=%g free_inodes_pct=%g fsp.fsu_blocksize=%"PRIu64" mult=%"PRIu64"\n",
  ...
  if (p->group == NULL) {
    ...
    for (p_list = path_select_list; p_list; p_list=p_list->name_next) {
      ...
      if (p_list->group && ! (strcmp(p_list->group, p->group))) {
        ...
        if (verbose >= 3)
          sprintf("Group %s: adding %lu blocks sized %"PRIu64", (%s) used_units=%g free_units=%g total_units=%g fsu_blocksize=%"PRIu64" mult=%"PRIu64"\n",
      ...
      if (verbose >= 3)
        sprintf("Group %s now has: used_units=%g free_units=%g total_units=%g fsu_blocksize=%"PRIu64" mult=%"PRIu64"\n",
